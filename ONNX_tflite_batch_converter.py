...
def get_representative_data(expected_shape, num_samples=100):
    """
    Dynamically formats the calibration dataset to match whatever 
    input shape onnx2tf decided to generate.
    """
    ...
    def gen():
        for i in range(num_samples):
            window = features[i : i + SEQUENCE_LENGTH] # Shape: (10, 9)
            if len(window) < SEQUENCE_LENGTH: 
                break
            
            # Start with PyTorch's native shape: (1, 9, 10, 1) -> (Batch, Channels, Seq, Width)
            window_t = np.transpose(window)
            base_shape = np.reshape(window_t, (1, 9, SEQUENCE_LENGTH, 1)).astype(np.float32)
            
            # If onnx2tf transposed the model to TF's native NHWC (1, 10, 1, 9)
...
def run_conversion_pipeline():
    ...
    for filename in onnx_files:
        ...
        strategies =['float32', 'hybrid', 'float16', 'int8']
        ...
        for strat in strategies:
            ...
            if strat == 'hybrid':
                ...
                converter.optimizations = [tf.lite.Optimize.DEFAULT]
                # Pass the dynamic shape we discovered into the calibrator
                converter.representative_dataset = get_representative_data(expected_input_shape)
